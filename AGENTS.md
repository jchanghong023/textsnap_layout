...
- 本仓库用于实现仅支持 Windows 11 x86-64（x64）、Intel Core i7-13700 的离线
  截图 OCR 工具 TextSnap Layout。
- 当前已完成计划范围内的应用代码、测试、依赖锁和 Windows x64 构建流水线；
  最终交付仍受指定 Windows 11 x64 实机验收约束。
...
- 构建、自动化测试、真实 OCR 回归和原生行为验收只支持 Windows 11
  x86-64（x64）、Intel Core i7-13700；不支持 Linux、Windows ARM64 或其他 ARM
  主机。
...
- `src/textsnap/ocr.py` 封装 PaddleOCR 预/后处理、ONNX Runtime 推理、瓦片和识别流程；
...
- Windows 专用导入和系统调用应隔离，使纯布局、分块、去重、状态机和配置测试
  保持确定性且不依赖 Win32 运行时状态。
...
- 构建脚本只需适用于 Windows 11 x64、Intel i7-13700 主机和 win-x64 目标；
  必须按锁文件安装 Windows 平台依赖，不得解析为其他平台产物。
...
- 真实 OCR 回归只在目标 Windows 11 x64、Intel i7-13700 环境运行，并使用与
  发布包相同的运行时、模型和参数；其他平台的结果不属于项目支持或验收范围。

## 当前命令约定

- 不产生缓存的静态检查：
  `ruff check --no-cache app native scripts src tests`
- Windows 11 x64 纯 Python 与静态回归：
  `$env:PYTHONPATH='src'; py -3.13 -B -m unittest discover -s tests -v`
- 安装 PySide6 Essentials 6.11.1 与 NumPy 2.2.6 后的 Qt 回归：
  `$env:QT_QPA_PLATFORM='offscreen'; $env:PYTHONPATH='src'; py -3.13 -B -m unittest tests.test_qt_instance tests.test_qt_worker tests.test_ui_widgets tests.test_controller -v`
- 锁文件结构与哈希校验：
  `py -3.13 -B -m scripts.build_release validate-lock --lock-dir vendor-lock`
- 真实 OCR 回归及所需环境变量见 `README.zh-CN.md`；只能在目标 Windows 11
  x64、Intel i7-13700 环境执行。
- 非交互构建入口为 `py -3.13 -B -m scripts.build_release`，支持
...
- staging 固定使用 `stage --profile private-use`，可以在后续 `verify` 后执行
  `package` 生成个人使用 ZIP。
- 命令依赖的 PySide6、OCR 推理框架、精确 CPython 3.13.14、MinGW、缓存、模型和字体
  不得假定已安装；执行前按 `README.zh-CN.md` 检查实际环境。

## 当前验证基线

以下是 2026-08-20 实际取得的回归基线，用于判断后续变更是否退化，不代表修改后
可以免于重跑：

- Windows 11 x64 上 `unittest discover` 共发现 299 项，结果为 `OK`，其中 3 项
  因可选环境条件明确跳过。
- Qt 真实点击事件和 Windows 原生平台渲染已验证“复制全部”后复制原文、关闭窗口并释放结果引用；局部复制保持窗口打开。
- 分发 staging 锁定了 70 个 wheel、4 个运行时资源、两套 ONNX 模型和 116 条有效
  依赖边；独立静态验证检查了 29,020 个条目和 244 个 PE。
- 从 staging 精确依赖栈完成 ONNX Runtime CPU 模型初始化和空白图真实推理。
- 静态 PE 检查仍保留 33 条 `load_path_pending`，全部为“待 Windows 加载器实机
  验证”。
...
4. 已区分 Windows 自动化验证项、静态验证项和待 Windows 实机交互验证项。
