...
- 当前存在发布合规门禁：2026-07-25 的最新静态 staging 记录了 18 个未解除项，
  其中 PaddleX 3.7.2 的传递依赖 `aistudio-sdk` 0.3.8 因无可核验的再分发许可
  明确阻断，另有 17 个许可证、notice 或许可关联审核项待处理。可以继续代码和
  测试工作，并可生成明确标记为不可分发的测试 staging；但在所有门禁解除前
  不得生成最终 ZIP，也不得声称许可证收集已通过。
...
- `scripts/build_release.py` 是非交互 CLI，具体锁校验、staging、许可证、
...
- 非分发 staging 锁定了 68 个 wheel、4 个运行时资源、两套模型和 113 条有效
  依赖边；独立静态验证检查了 28,906 个条目和 241 个 PE。
- 静态 PE 检查仍保留 29 条 `load_path_pending`，全部为“待 Windows 加载器实机
  验证”；许可证门禁包含上述 18 个未解除项。
- `package` 已验证会以许可证门禁状态码 3 拒绝，并且不会创建输出目录或最终 ZIP。
